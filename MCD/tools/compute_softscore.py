...
def add_multiple_choice_answers_to_dataset(dataset):
    for sample in dataset:
        # Count the frequency of each 'answer' in the sample's 'answers' list
        answer_counter = Counter(
            answer_dict['answer'] for answer_dict in sample['answers']
        )
        # Find the most common 'answer'
        most_common_answer = answer_counter.most_common(1)[0][0]
        # Add the 'multiple_choice_answer' to each answer dictionary in the sample's 'answers' list
        sample['multiple_choice_answer'] = most_common_answer
...
if __name__ == '__main__':
    ...
    if config.version=='ok':
        print('calculate multiple_choice_answer')
        add_multiple_choice_answers_to_dataset(answers)
